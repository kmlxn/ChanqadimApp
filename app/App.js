...
class TodoList extends Component {
  ...
  completeList(list) {
    this.props.store.completeList(list)
  }
        ...
        <View style={{flex:1}}>
          {list.map((l, i) => {
            return <View key={i} style={styles.itemContainer}>
              ...
              <Text
                style={{flex: 1, backgroundColor: l.isCompleted ? 'red' : 'blue'}}
                onPress={this.completeList.bind(this, l)}>Complete {l.isCompleted}</Text>
            </View>
          })}
        </View>
        <TouchableHighlight
